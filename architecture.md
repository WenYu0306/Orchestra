...
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────┐ ┌──────────┐   │
│  │ JD 评分   ││ Agent调度 │ │ 招呼语   │ │假检测│ │ 搜索词   │   │
│  │ (0-100)  │ │观察→决策→ │ │ 动态生成  │ │      │ │ 动态生成  │   │
│  │          │ │执行循环   │ │          │ │      │ │          │   │
│  └──────────┘ └──────────┘ └──────────┘ └──────┘ └──────────┘   │
├──────────────────────────────────────────────────────────────────┤
│          执行层 · 数据获取                                         │
...
│  │启动Chrome│ │insert_text│ │ 调BOSS API│ │--disable-blink     │   │
│  │          │ │input_     │ │joblist/card│ │nodriver小众库     │   │
│  └──────────┘ └──────────┘ └──────────┘ └────────────────────┘   │
└──────────────────────────────────────────────────────────────────┘

数据流：Chrome → XHR 搜 joblist.json → 标签评分 → 过滤 → top 30 详情
重评 → Agent 自主调度 → 分层 → SSE 推送前端 → 勾选 → XHR friend/add.json + CDP insert_text 自定义招呼语
```
