...
const pill = computed(() => {
  switch (props.status) {
    ...
    case 'need_captcha': return { cls: 'captcha', text: '遇到验证码 · 等待处理' }
    case 'completed': return { cls: 'done', text: `已完成 · 共匹配 ${props.matched} 个` }
  ...
  <header class="top-bar glass">
    ...
    <div class="ctrl-wrap">
      ...
      <button class="btn btn-stop" :disabled="!isRunning" @click="emit('stop')">
        <span class="ico-stop"></span><span>停止</span>
      </button>
      <button class="btn btn-start" :disabled="isRunning" @click="emit('start')">
        <svg class="ico-start" viewBox="0 0 14 14" width="14" height="14"><path d="M4 2.5 L11 7 L4 11.5 Z" fill="currentColor"/></svg>
        <span>开始</span>
      </button>
    </div>
  </header>
...
.logo-text { font-family:var(--font-en); font-weight:600; font-size:18px; color:var(--text-1); }
.title-wrap { flex:1; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:2px; }
...
.status-pill { display:flex; align-items:center; gap:8px; height:31px; padding:0 14px; border-radius:999px; border:1px solid transparent; }
.status-pill .dot { width:8px; height:8px; border-radius:999px; background:var(--text-3); }
.status-pill .status-text { font-size:12px; font-weight:500; }
.status-pill.running { background:var(--green-soft); border-color:var(--green-border); }
.status-pill.running .dot { background:var(--green); animation:pulse 1.8s ease-in-out infinite; }
...
@keyframes pulse { 0%,100%{box-shadow:0 0 0 0 rgba(16,185,129,0.5),0 0 6px 2px rgba(16,185,129,0.8)} 50%{box-shadow:0 0 0 6px rgba(16,185,129,0),0 0 12px 4px rgba(16,185,129,0.5)} }
.btn { display:flex; align-items:center; gap:6px; height:35px; padding:0 14px; border-radius:12px; font-size:13px; font-weight:600; transition:transform .2s,box-shadow .2s,opacity .2s; }
.btn:disabled { opacity:0.45; cursor:not-allowed; }
.btn:not(:disabled):hover { transform:translateY(-1px); }
.btn-stop { background:var(--red); color:#fff; box-shadow:0 4px 12px rgba(239,68,68,0.25),inset 0 1px 0 rgba(255,255,255,0.2); }
.ico-stop { width:10px; height:10px; background:#fff; border-radius:2px; }
.btn-start { background:#F0F1F5; color:var(--text-3); border:1px solid #E0E1E8; }
