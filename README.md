...
AI Agent —— BOSS 直聘全自动求职：搜索 → 评分 → 分层 → 生成招呼语 → 一键发送。
...
| SSE 实时推送 | ✅ 前端逐条显示卡片 |
| 前端勾选发送 | ✅ 复选框 + 发送选中按钮 |
| 真发送 | ✅ XHR 调 friend/add.json 成功 |
| 自定义招呼语 | ⚠️ BOSS 默认招呼语，自定义待补 |

## 架构

三层 — 连接层（nodriver + CDP + Chrome + XHR）、执行层（httpx → DeepSeek）、决策层（agent_runner 编排）。
